// Rename this file secrets.h
// Enter your Wifi and OpenAI API key below
//
const char *ssid = "";
const char *password = "";
const char *chatGPT_APIKey = "";

String apiKeyOpenWeather = ""; // Obtain this from your OpenWeather account
#define GITHUBURL "https://raw.githubusercontent.com/bill-orange/Soil-Resistance-Transmitter/master/data"
String LAT =  ""; // 90.0000 to -90.0000 negative for Southern hemisphere
String LON = ""; // 180.000 to -180.000 negative for West

String Units = "l";  // or "imperial"
String Language = "";   // See notes tab

// Replace with your country code and city
String location = "";

